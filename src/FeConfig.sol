...
contract FeConfig {
    ...
    function with_code(string memory code_) public returns (FeConfig) {
        code = code_;
        return this;
    }
    ...
    function with_deployer(address _deployer) public returns (FeConfig) {
        deployer = _deployer;
        return this;
    }

    /// @notice sets the evm version to compile with
    function with_evm_version(string memory _evm_version) public returns (FeConfig) {
    ...
    function with_constant(string memory key, string memory value_) public returns (FeConfig) {
    ...
    function with_addr_constant(string memory key, address value_) public returns (FeConfig) {
    ...
    function with_bytes32_constant(string memory key, bytes32 value_) public returns (FeConfig) {
        const_overrides.push(Constant(key, bytesToString(abi.encodePacked(value_))));
        return this;
    }

    /// @notice sets a constant to a uint256 value in the current compilation environment
    function with_uint_constant(string memory key, uint256 value_) public returns (FeConfig) {
        const_overrides.push(Constant(key, bytesToString(abi.encodePacked(value_))));
        return this;
    }
    ...
    /// @notice Checks for Fe binary conflicts
    function binary_check() public {
        string[] memory bincheck = new string[](1);
        bincheck[0] = "./lib/foundry-fe/scripts/binary_check.sh";
    ...
    function creation_code(string memory file) public payable returns (bytes memory bytecode) {
        ...
        time[0] = "./lib/foundry-fe/scripts/rand_bytes.sh";
        ...
        create_cmds[0] = "./lib/foundry-fe/scripts/file_writer.sh";
        create_cmds[1] = string.concat("src/", tempFile, ".fe");
        create_cmds[2] = string.concat(code, "\n");
        vm.ffi(create_cmds);

        // Append the real code to the temp file
        string[] memory append_cmds = new string[](3);
        append_cmds[0] = "./lib/foundry-fe/scripts/read_and_append.sh";
        append_cmds[1] = string.concat("src/", tempFile, ".fe");
        append_cmds[2] = string.concat("src/", file, ".fe");
        vm.ffi(append_cmds);
    ...
    function deploy(string memory file) public payable returns (address) {
        ...
        require(deployedAddress != address(0), "FeDeployer could not deploy contract");
